...
:: ============================================
:: 3. Portproxy + Firewall (sadece sunucu bu PC'yse)
:: ============================================
echo  [3/3] Portproxy kontrol ediliyor...
...
if not errorlevel 1 (
    ...
    if errorlevel 1 (
        echo         Portproxy eksik, ekleniyor (izin gerekebilir)...
        powershell -NoProfile -ExecutionPolicy Bypass -Command ^
        ...
        echo         Portproxy zaten aktif.
    )
) else (
    echo         Bu PC sunucu degil, portproxy atlanıyor.
)
echo  [3/3] OK
