...
| **Domain (PC)** | http://restaurantos.local _(Setup-Hosts.bat gerekli)_ |
| **Mobil**       | Giriş ekranındaki **QR kodu** tarayın                 |

### PC'de Domain Adı Aktif Etme (bir kez)

Sunucu bilgisayarda `Setup-Hosts.bat` dosyasına sağ tık → **Yönetici olarak çalıştır**. Bu işlem sonrası tarayıcıdan `http://restaurantos.local` yazarak erişebilirsiniz.

### Ağdaki Diğer Bilgisayarlardan Kolay Erişim

1. Bu klasördeki **`Agdan-Baglan.bat`** dosyasını flash bellek ile diğer bilgisayara kopyalayın.
2. Sağ tıklayıp **Yönetici olarak çalıştırın**.
3. Ağdaki sunucuyu otomatik bulacak ve o bilgisayarda da `http://restaurantos.local` adresinin çalışmasını sağlayacaktır.
